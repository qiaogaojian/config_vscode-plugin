...
class SnippProvider {
    ...
    getTreeItem(element) {
        ...
        let icn = path_1.join(__filename, "..", "..", "..", "resources", "icons", `folder-${t}.svg`);
        const isSnip = this.isSnipp(element);
        if (isSnip) {
            const it = element.contentType;
            icn = path_1.join(__filename, "..", "..", "..", "resources", "icons", `${it}.svg`);
        }
        const snippcomm = {
...
class SnippExplorer {
    constructor(context) {
        ...
        vscode.commands.registerCommand("allSnipps.editEntry", (snipp) => {
            ...
            panel.webview.html = edit_snipp_1.default(snipp);
