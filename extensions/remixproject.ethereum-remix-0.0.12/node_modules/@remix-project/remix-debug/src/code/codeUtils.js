...
const opcodes_2 = tslib_1.__importDefault(require("./opcodes"));
function nameOpCodes(raw, hardfork) {
    const common = new common_1.default({ chain: 'mainnet', hardfork });
    const opcodes = opcodes_1.getOpcodesForHF(common);
    let pushData = '';
    const codeMap = {};
    const code = [];
...
function parseCode(raw) {
    const common = new common_1.default({ chain: 'mainnet', hardfork: 'berlin' });
    const opcodes = opcodes_1.getOpcodesForHF(common);
    const code = [];
    for (let i = 0; i < raw.length; i++) {
        ...
        try {
            ...
            const opcodeDetails = opcodes_2.default(raw[i], false);
