...
function getMappingLocation(key, position) {
    ...
    const mappingK = ethereumjs_util_1.toBuffer(ethereumjs_util_1.addHexPrefix(key));
    let mappingP = ethereumjs_util_1.toBuffer(ethereumjs_util_1.addHexPrefix(position));
    mappingP = ethereumjs_util_1.setLengthLeft(mappingP, 32);
    const mappingKeyBuf = concatTypedArrays(mappingK, mappingP);
    const mappingStorageLocation = ethereumjs_util_1.keccak(mappingKeyBuf);
