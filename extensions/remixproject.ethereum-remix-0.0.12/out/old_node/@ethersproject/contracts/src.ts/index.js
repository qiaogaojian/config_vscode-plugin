...
function populateTransaction(contract, fragment, args) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const resolved = yield properties_1.resolveProperties({
        ...
        if (tx.gasLimit == null && fragment.gas != null) {
            ...
            let intrinsic = 21000;
            const bytes = bytes_1.arrayify(data);
...
function buildCall(contract, fragment, collapseSimple) {
    ...
    return function (...args) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (args.length === fragment.inputs.length + 1 && typeof (args[args.length - 1]) === "object") {
                const overrides = properties_1.shallowCopy(args.pop());
            ...
            tx.wait = (confirmations) => {
                return wait(confirmations).then((receipt) => {
                    receipt.events = receipt.logs.map((log) => {
                        let event = properties_1.deepCopy(log);
                        let parsed = null;
                        try {
...
class FragmentRunningEvent extends RunningEvent {
    constructor(address, contractInterface, fragment, topics) {
        ...
        properties_1.defineReadOnly(this, "fragment", fragment);
    }
    prepareEvent(event) {
    ...
    getEmit(event) {
        const errors = abi_1.checkResultErrors(event.args);
        if (errors.length) {
...
class BaseContract {
    constructor(addressOrName, contractInterface, signerOrProvider) {
        ...
        properties_1.defineReadOnly(this, "interface", properties_1.getStatic((new.target), "getInterface")(contractInterface));
        ...
        properties_1.defineReadOnly(this, "filters", {});
        {
            ...
            Object.keys(this.interface.events).forEach((eventSignature) => {
                ...
                properties_1.defineReadOnly(this.filters, eventSignature, (...args) => {
                    return {
            ...
            Object.keys(uniqueFilters).forEach((name) => {
                ...
                if (filters.length === 1) {
                    properties_1.defineReadOnly(this.filters, name, this.filters[filters[0]]);
                }
                else {
        ...
        Object.keys(this.interface.functions).forEach((signature) => {
            ...
            if (this[signature] == null) {
                properties_1.defineReadOnly(this, signature, buildDefault(this, fragment, true));
            }
            // We do not collapse simple calls on this bucket, which allows
            // frameworks to safely use this without introspection as well as
            // allows decoding error recovery.
            if (this.functions[signature] == null) {
                properties_1.defineReadOnly(this.functions, signature, buildDefault(this, fragment, false));
            }
            if (this.callStatic[signature] == null) {
                properties_1.defineReadOnly(this.callStatic, signature, buildCall(this, fragment, true));
            }
            if (this.populateTransaction[signature] == null) {
                properties_1.defineReadOnly(this.populateTransaction, signature, buildPopulate(this, fragment));
            }
            if (this.estimateGas[signature] == null) {
                properties_1.defineReadOnly(this.estimateGas, signature, buildEstimate(this, fragment));
            }
        });
        Object.keys(uniqueNames).forEach((name) => {
            ...
            try {
                if (this[name] == null) {
                    properties_1.defineReadOnly(this, name, this[signature]);
                }
            }
            catch (e) { }
            if (this.functions[name] == null) {
                properties_1.defineReadOnly(this.functions, name, this.functions[signature]);
            }
            if (this.callStatic[name] == null) {
                properties_1.defineReadOnly(this.callStatic, name, this.callStatic[signature]);
            }
            if (this.populateTransaction[name] == null) {
                properties_1.defineReadOnly(this.populateTransaction, name, this.populateTransaction[signature]);
            }
            if (this.estimateGas[name] == null) {
                properties_1.defineReadOnly(this.estimateGas, name, this.estimateGas[signature]);
            }
        });
    }
    ...
    fallback(overrides) {
        ...
        const tx = properties_1.shallowCopy(overrides || {});
    ...
    _wrapEvent(runningEvent, log, listener) {
        const event = properties_1.deepCopy(log);
    ...
    queryFilter(event, fromBlockOrBlockhash, toBlock) {
        ...
        const filter = properties_1.shallowCopy(runningEvent.filter);
...
class ContractFactory {
    ...
    deploy(...args) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const address = properties_1.getStatic(this.constructor, "getContractAddress")(tx);
            const contract = properties_1.getStatic(this.constructor, "getContract")(address, this.interface, this.signer);
