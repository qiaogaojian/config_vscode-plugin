...
function _decrypt(data, key, ciphertext) {
    const cipher = utils_1.searchPath(data, "crypto/cipher");
    if (cipher === "aes-128-ctr") {
        const iv = utils_1.looseArrayify(utils_1.searchPath(data, "crypto/cipherparams/iv"));
...
function _getAccount(data, key) {
    const ciphertext = utils_1.looseArrayify(utils_1.searchPath(data, "crypto/ciphertext"));
    const computedMAC = bytes_1.hexlify(keccak256_1.keccak256(bytes_1.concat([key.slice(16, 32), ciphertext]))).substring(2);
    ...
    const address = transactions_1.computeAddress(privateKey);
    if (data.address) {
    ...
    if (utils_1.searchPath(data, "x-ethers/version") === "0.1") {
        const mnemonicCiphertext = utils_1.looseArrayify(utils_1.searchPath(data, "x-ethers/mnemonicCiphertext"));
        const mnemonicIv = utils_1.looseArrayify(utils_1.searchPath(data, "x-ethers/mnemonicCounter"));
        ...
        const path = utils_1.searchPath(data, "x-ethers/path") || hdnode_1.defaultPath;
        const locale = utils_1.searchPath(data, "x-ethers/locale") || "en";
        const entropy = bytes_1.arrayify(mnemonicAesCtr.decrypt(mnemonicCiphertext));
        try {
            const mnemonic = hdnode_1.entropyToMnemonic(entropy, locale);
            const node = hdnode_1.HDNode.fromMnemonic(mnemonic, null, locale).derivePath(path);
...
function _computeKdfKey(data, password, pbkdf2Func, scryptFunc, progressCallback) {
    const passwordBytes = utils_1.getPassword(password);
    const kdf = utils_1.searchPath(data, "crypto/kdf");
    if (kdf && typeof (kdf) === "string") {
        const throwError = function (name, value) {
            return logger.throwArgumentError("invalid key-derivation function parameters", name, value);
        };
        if (kdf.toLowerCase() === "scrypt") {
            const salt = utils_1.looseArrayify(utils_1.searchPath(data, "crypto/kdfparams/salt"));
            const N = parseInt(utils_1.searchPath(data, "crypto/kdfparams/n"));
            const r = parseInt(utils_1.searchPath(data, "crypto/kdfparams/r"));
            const p = parseInt(utils_1.searchPath(data, "crypto/kdfparams/p"));
            ...
            const dkLen = parseInt(utils_1.searchPath(data, "crypto/kdfparams/dklen"));
            if (dkLen !== 32) {
            ...
            const salt = utils_1.looseArrayify(utils_1.searchPath(data, "crypto/kdfparams/salt"));
            let prfFunc = null;
            const prf = utils_1.searchPath(data, "crypto/kdfparams/prf");
            if (prf === "hmac-sha256") {
                prfFunc = "sha256";
            }
            ...
            const count = parseInt(utils_1.searchPath(data, "crypto/kdfparams/c"));
            const dkLen = parseInt(utils_1.searchPath(data, "crypto/kdfparams/dklen"));
            if (dkLen !== 32) {
                throwError("dklen", dkLen);
            }
...
function encrypt(account, password, options, progressCallback) {
    try {
        ...
        if (hasMnemonic(account)) {
            ...
            const node = hdnode_1.HDNode.fromMnemonic(mnemonic.phrase, null, mnemonic.locale).derivePath(mnemonic.path || hdnode_1.defaultPath);
            if (node.privateKey != account.privateKey) {
                throw new Error("mnemonic mismatch");
            }
        }
    ...
    const privateKey = bytes_1.arrayify(account.privateKey);
    const passwordBytes = utils_1.getPassword(password);
    let entropy = null;
    let path = null;
    let locale = null;
    ...
    return scrypt_js_1.default.scrypt(passwordBytes, salt, N, r, p, 64, progressCallback).then((key) => {
        ...
        const ciphertext = bytes_1.arrayify(aesCtr.encrypt(privateKey));
        // Compute the message authentication code, used to check the password
        const mac = keccak256_1.keccak256(bytes_1.concat([macPrefix, ciphertext]));
        ...
        if (entropy) {
            const mnemonicIv = random_1.randomBytes(16);
            const mnemonicCounter = new aes_js_1.default.Counter(mnemonicIv);
            const mnemonicAesCtr = new aes_js_1.default.ModeOfOperation.ctr(mnemonicKey, mnemonicCounter);
            const mnemonicCiphertext = bytes_1.arrayify(mnemonicAesCtr.encrypt(entropy));
            const now = new Date();
