...
function formatNumber(value, name) {
    const result = bytes_1.stripZeros(bignumber_1.BigNumber.from(value).toHexString());
    if (result.length > 32) {
...
function _parseEip2930(payload) {
    ...
    try {
        const digest = keccak256_1.keccak256(_serializeEip2930(tx));
...
function parse(rawTransaction) {
    const payload = bytes_1.arrayify(rawTransaction);
    // Legacy and EIP-155 Transactions
    if (payload[0] > 0x7f) {
